...
Это означает:
- Слот воскресенья 26.04 23:00 был открыт уже как минимум с 24.04 (а скорее всего — с 24.04 утра, когда впервые стало видно «сегодня + 2 дня вперёд» = 26.04).
- HTML-инжекшн в `/api/v1/bookform/551098/` указывает на ограничение клуба: "Мы принимаем бронирования на ближайшие три дня" — где «три дня» = today + 2 forward = 3 даты в выдаче.
- **Эмпирически подтверждено (production):** booking horizon = `today + 2 calendar days` (включая today). Слот D становится `is_bookable: true` в день D−2 ровно в 07:00 Asia/Almaty. Под это значение настроен `LEAD_DAYS = 2` в `scheduler/window.py`. Прежнее значение `LEAD_DAYS = 3` приводило к холостому fire за 24 часа до реального открытия (бот полагался на poll fallback, теряя время).
