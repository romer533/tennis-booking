...
- Эти заголовки реализуют антибот-защиту Altegio. Без них POST скорее всего будет отвергнут (статус неизвестен — провокация требуется).
...
- Зависимость от cookie: **в HAR нет ни одной cookie во всём флоу** — значит сервер не требует state, выставленного предыдущими GET. Это сильный аргумент в пользу того, что POST можно стрелять «холодно», без bootstrap-разогрева. **НО**: антибот-токены `x-app-signature`/`x-app-client-context` всё равно надо генерить. Они не зависят от cookies, но зависят от JS-бандла версии `199620.2b0fce8b` — алгоритм может меняться при обновлении фронта.
...
1. **POST /api/v1/book_record/521176** с body вида:
   ...
   ```
   с headers `x-app-signature` + `x-app-client-context` (генерация неизвестна, см. блок "Антибот"), без cookie.

Никаких прогревочных GET-запросов сервер НЕ требует (cookies во всём флоу — ноль). Но антибот-токены, скорее всего, требуются.
...
- **Свежие `x-app-signature` и `x-app-client-context`** — это ключевая неизвестная.

Cookie/токены по-сессии заранее получать НЕ нужно — их просто нет.
...
- **Антибот-защита:** реализована через `x-app-signature` (32-байт hex, похоже на HMAC/SHA-256) и `x-app-client-context` (формат `<16-char-prefix>:<~500-char base64>`, скорее всего AES-CBC/GCM зашифрованный JSON с fingerprint+timestamp). Шлются ТОЛЬКО на `POST /api/v1/book_record/...`. Все остальные запросы (включая `attendances/calculate`) шлют их пустыми, и сервер не возражает — то есть это ИМЕННО защита write-операции.
- **`x-altegio-application-*` headers (name/platform/version):** статичные строки, идентифицирующие фронт. `version: 199620.2b0fce8b` — менется при деплое нового бандла. Скорее всего сервер их слабо валидирует (формат проверяет, конкретные значения — нет).
- **Если опустить Origin/Referer:** не проверено, но `Origin: https://b551098.alteg.io` стандартно для same-origin POST. Скорее всего сервер проверяет минимум `origin == referer.origin`.

**Можно ли разогреть заранее:** не нужно — раз cookies/auth нет. Но антибот-токены, видимо, "одноразовые" для конкретного запроса (sign включает body). То есть генерить их нужно прямо в момент POST. **Это плохо для hot-path:** мы не можем подготовить токен за минуту до T−0; либо мы реверсим JS-алгоритм генерации, либо запускаем headless-браузер в момент атаки.
...
### A. Провокации, которые нужно сделать вручную

1. **Закрытый слот (T < открытие окна).** Через DevTools / Postman повторить `POST /book_record` с `datetime` через 4+ дня вперёд (за горизонтом 3 дней). Зафиксировать: статус (404? 422? 200 с error в теле?), точное тело ответа, есть ли `error_code` / `meta`. Это критично для retry-стратегии engine — мы должны различать "слот ещё не открылся, повторять каждые 200ms" от "слот занят навсегда, бросаем".

2. **Занятый слот.** Сразу после создания брони повторить тот же POST (тот же staff_id, datetime). Что вернёт? 409? 422? 200 с `success: false`? Точный shape ответа.

3. **Дубликат.** Запустить два одинаковых POST с интервалом 50ms (тот же staff/datetime/phone). Получится две брони, одна+ошибка, или обе пройдут?

4. **Без антибот-токенов.** Через cURL повторить успешный POST, опустив `x-app-signature` и `x-app-client-context`. Пройдёт? Если нет — какой статус и тело? Это определит, можем ли мы вообще отказаться от реверсинга JS-генерации токенов.

5. **Без Origin/Referer.** То же, без этих headers. Если пройдёт — антибот завязан только на `x-app-signature`.

6. **Параллелизм.** N ∈ {3, 5, 10} одновременных POST на РАЗНЫЕ тестовые слоты (можно с разными staff_id). Все пройдут? Какой-то порог 429? `Retry-After`?

7. **search/timeslots до открытия окна.** Запросить `search/dates` с `date_from = T+4` (явно за горизонтом). Вернёт массив с `is_bookable: false` или сразу пустой? Можно ли поллить именно через search/timeslots с конкретной датой и стейкаут момент перехода?

8. **Точный момент открытия окна.** Запустить `search/dates` каждые 5 секунд в окрестности `06:55:00..07:05:00 Asia/Almaty` целевого дня (например, 25.04 в 06:55 — должна "открыться" 28.04 = next-Monday slot). Зафиксировать timestamp перехода `is_bookable: false → true`.

### B. Архитектурные вопросы по результатам HAR

1. **Реверсинг `x-app-signature` / `x-app-client-context` vs. headless-браузер.** Без них POST скорее всего отвергается (провокация подтвердит). Два пути:
   - **Реверс JS:** скачать бандл `199620.2b0fce8b`, найти функции генерации токенов, переписать на Python. Плюс: моментальный POST из чистого HTTP-клиента, минимум latency. Минус: ломается при каждом обновлении бандла Altegio (а бандл версии есть в URL — значит, обновляется регулярно). Поддержка эксплойта = дополнительная работа на каждый релиз фронта.
   - **Headless-браузер (Playwright):** запускаем headless Chromium, открываем `b551098.alteg.io`, доходим до точки, где можно сделать `fetch('/api/v1/book_record/521176', {body, headers})` через `page.evaluate`. Плюс: токены генерируются "родным" JS, не ломается при обновлениях. Минус: latency 1-3 секунды на bootstrap, plus headless detection.
   - **Гибрид:** держать длящийся headless-браузер с открытой страницей, и в момент T−0 вызвать `page.evaluate(() => fetch(...))`. Bootstrap делается заранее (за 30 секунд), POST идёт мгновенно.
   - **Решение:** пока не делаем выбор. После провокации #4 (без токенов) поймём, обязательны ли они вообще.

2. **«Холодная» стрельба.** Cookies нет, сессии нет — значит prewarm не нужен с точки зрения state. Но антибот-токены нужно сгенерировать — это сразу делает hot-path не "холодным". Уточнить: TTL у `x-app-client-context`. Можно ли сгенерить за 10 секунд до POST и переиспользовать? Это требует реверсинга или эмпирической проверки.
...
- [x] Понятно, какие токены/cookies нужны: cookies — НИКАКИЕ, токены — `x-app-signature` + `x-app-client-context` на сам POST. **TTL и алгоритм генерации не известны.**
- [x] Понятно, требуется ли SMS / captcha: нет (но провокация без антибот-токенов может вызвать captcha — открытый вопрос #4).
- [ ] **Не закрыто:** что вернёт API на запрос ДО открытия окна. Требуется провокация #1.
- [ ] **Не закрыто:** rate-limit при N параллельных запросов. Требуется провокация #6.
- [ ] **Не закрыто:** идемпотентность booking-POST. Требуется провокация #3.

**Phase 0 не должна закрываться, пока провокации #1, #3, #4, #6 не выполнены и результаты не задокументированы в этом файле.**
